...
const lib_2 = require("@a-a-game-studio/aa-redis-sys/lib");
...
class App {
    constructor(conf, iPort = 3005) {
        ...
        this.objExpress = express_1.default(); // уст. Express
        ...
        this.objExpress.use((req, response, next) => {
            ...
            req.sys.errorSys = new lib_1.Components.ErrorSys(this.conf.common.env);
            next();
        });
        /* запрос */
    ...
    fUseMySql() {
        ...
        ;
        this.objDb = knex_1.default(this.conf.mysql);
        this.objExpress.use((req, resp, next) => {
     ...
     * заменяет redis
     */
    fUseSharedMem(globalMem) {
        this.objExpress.use((req, resp, next) => {
            req.infrastructure.redis = new lib_2.SharedMemSys.SharedMemSys(globalMem);
            req.sys.cacheSys = new CacheSys_1.CacheSys(req); // Система кеширования
            next();
        }); // уст. конфиг
        this.bUseReddis = true;
     ...
     */
    fUseStatic(sPath) {
        this.objExpress.use(express_1.default.static(sPath));
        return this;
    }
    /**
    ...
    fUseFileModule() {
        ...
        this.objExpress.use(express_1.default.static(this.conf.FileModule.sUrl));
