...
class CacheSys {
     ...
     * @param callback - функция получающая данные из БД
     */
    async autoCacheList(sKey, iTimeSec, callback) {
        let ok = this.errorSys.isOk();
        let bCache = false; // Наличие кеша
        let sCache = null;
        let out = null;
        ...
        if (ok && !bCache) { // Если значения нет в кеше - добавляем его в кеш
            out = await callback();
            if (out && (util_1.isArray(out))) {
                this.redisSys.set(sKey, JSON.stringify(out), iTimeSec);
            }
            else {
                this.errorSys.devNotice(sKey, 'Не удалось посместить значение в кеш не является массивом');
            }
        }
        if (ok && bCache) { // Если значение взято из кеша - отдаем его в ответ
            out = JSON.parse(sCache);
    ...
    async autoCacheID(sKey, iTimeSec, callback) {
        ...
        if (ok && !bCache) { // Если значения нет в кеше - добавляем его в кеш
            ...
            if (out || out > 0) {
                ...
                this.errorSys.devWarning(sKey, 'Неверный тип, должен быть (number и > 0) => ' + out);
            }
        }
        if (ok && bCache) { // Если значение взято из кеша - отдаем его в ответ
            out = Number(sCache);
