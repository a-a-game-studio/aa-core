 ...
 * Войти в систему
 */
router.post(R.login.route, async (req, res, next) => {
    const ctrl = new UserController(req, res);
    await ctrl.faInit();
 ...
 * Зарегистрироваться
 */
router.post(R.register.route, async (req, res, next) => {
    const ctrl = new UserController(req, res);
    await ctrl.faInit();
