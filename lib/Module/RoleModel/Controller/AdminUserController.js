 ...
 */
router.post('/api/admin/user/get-users', async (req, res, next) => {
 ...
 */
router.post('/api/admin/user/get-user', async (req, res, next) => {
    let self = await AdminUserController.Init(req, res);
    let ok = self.userSys.isAccessRead(); // Проверка доступа
    let out = null;
 ...
 */
router.post('/api/admin/user/get-group', async (req, res, next) => {
    let self = await AdminUserController.Init(req, res);
    let ok = self.userSys.isAccessRead(); // Проверка доступа
    let out = null;
 ...
 */
router.post('/api/admin/user/get-ctrl-access', async (req, res, next) => {
    let self = await AdminUserController.Init(req, res);
    let ok = self.userSys.isAccessRead(); // Проверка доступа
    let out = null;
 ...
 */
router.post('/api/admin/user/get-user-groups', async (req, res, next) => {
    let self = await AdminUserController.Init(req, res);
    let ok = self.userSys.isAccessRead(); // Проверка доступа
    let out = null;
 ...
 */
router.post('/api/admin/user/get-ctrl-access-of-group', async (req, res, next) => {
    let self = await AdminUserController.Init(req, res);
    let ok = self.userSys.isAccessRead(); // Проверка доступа
    let out = null;
 ...
 */
router.post('/api/admin/user/get-group-list', async (req, res, next) => {
    let self = await AdminUserController.Init(req, res);
    let ok = self.userSys.isAccessRead(); // Проверка доступа
    let out = null;
 ...
 */
router.post('/api/admin/user/get-ctrl-access-list', async (req, res, next) => {
    let self = await AdminUserController.Init(req, res);
    let ok = self.userSys.isAccessRead(); // Проверка доступа
    let out = null;
    if (ok) { // Получаем весь список ролей
...
// ========================================================
/**
 * @Route("/api/admin/user/add-group", name="api__admin__user__add_ctrl_access")
 */
router.post('/api/admin/user/add-group', async (req, res, next) => {
    let self = await AdminUserController.Init(req, res);
    let ok = self.userSys.isAccessCreate(); // Проверка доступа
    let out = null;
    if (ok) { // Добавляем пользователя
        try {
            out = await self.groupM.addGroup(req.body);
        }
        catch (e) {
            self.errorSys.errorEx(e, 'fatal_error', 'Фатальная ошибка');
        }
    }
    res.send(self.responseSys.response(out, 'Добавленна группа'));
});
/**
 * @Route("/api/admin/user/add-ctrl-access", name="api__admin__user__add_ctrl_access")
 */
router.post('/api/admin/user/add-ctrl-access', async (req, res, next) => {
    let self = await AdminUserController.Init(req, res);
    let ok = self.userSys.isAccessCreate(); // Проверка доступа
    let out = null;
    if (ok) { // Добавляем пользователя
 ...
 */
router.post('/api/admin/user/add-user-to-group', async (req, res, next) => {
    let self = await AdminUserController.Init(req, res);
    let ok = self.userSys.isAccessCreate(); // Проверка доступа
    let out = null;
    if (ok) { // Добавляем пользователя
 ...
 */
router.post('/api/admin/user/add-ctrl-access-to-group', async (req, res, next) => {
    let self = await AdminUserController.Init(req, res);
    let ok = self.userSys.isAccessCreate(); // Проверка доступа
    let out = null;
 ...
 */
router.post('/api/admin/user/save-group', async (req, res, next) => {
    let self = await AdminUserController.Init(req, res);
 ...
 */
router.post('/api/admin/user/save-ctrl-access', async (req, res, next) => {
    let self = await AdminUserController.Init(req, res);
    let ok = self.userSys.isAccessUpdate(); // Проверка доступа
    let out = null;
 ...
 */
router.post('/api/admin/user/save-access-group', async (req, res, next) => {
    let self = await AdminUserController.Init(req, res);
    let ok = self.userSys.isAccessUpdate(); // Проверка доступа
    let out = null;
 ...
 */
router.post('/api/admin/user/del-group', async (req, res, next) => {
    let self = await AdminUserController.Init(req, res);
    let ok = self.userSys.isAccessDelete(); // Проверка доступа
    let out = null;
    if (ok) { // Удаляем пользователя
        try {
            out = await self.groupM.delGroup(req.body);
        }
        catch (e) {
            self.errorSys.errorEx(e, 'fatal_error', 'Фатальная ошибка');
        }
    }
    res.send(self.responseSys.response(out, 'Удалить группу'));
});
/**
 * @Route("/api/admin/user/del-user-from-group", name="api__admin__user__del_user_from_group")
 */
router.post('/api/admin/user/del-user-from-group', async (req, res, next) => {
    let self = await AdminUserController.Init(req, res);
    let ok = self.userSys.isAccessDelete(); // Проверка доступа
    let out = null;
    if (ok) { // Удаляем пользователя
 ...
 */
router.post('/api/admin/user/del-ctrl-access', async (req, res, next) => {
    let self = await AdminUserController.Init(req, res);
    let ok = self.userSys.isAccessDelete(); // Проверка доступа
    let out = null;
    if (ok) { // Удаляем пользователя
 ...
 */
router.post('/api/admin/user/del-ctrl-access-from-group', async (req, res, next) => {
    let self = await AdminUserController.Init(req, res);
    let ok = self.userSys.isAccessDelete(); // Проверка доступа
    let out = null;
