...
class UserGroupSQL extends BaseSQL_1.default {
    ...
    async getUserGroupsByUserID(idUser) {
        ...
        if (ok && !bCache) { // Получаем список ролей
            let sql = `
                ...
                JOIN ${GroupsE_1.GroupsE.NAME} pg ON pg.group_id = pug.group_id
                WHERE
                    pug.user_id = :user_id;
                ;
            `;
    ...
    async addUserToGroup(idUser, idGroup) {
        ...
        if (ok) { // Если пользователя в группе нет добавляем его в группу
            let sql = `
                INSERT INTO ${UserGroupE_1.UserGroupE.NAME}
            ...
            let resp = null;
            try {
                resp = await this.db(UserGroupE_1.UserGroupE.NAME).insert({
                    user_id: idUser,
                    group_id: idGroup,
                    group_leader: 0,
                    user_pending: 0
