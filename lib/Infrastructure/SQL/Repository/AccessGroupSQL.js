...
class AccessGroupSQL extends BaseSQL_1.default {
        ...
        if (ok) { // Получить список модулей доступных группе по ID Группы
            let sql = `
                SELECT
                    ...
                    ca.name,
                    ca.descript
                FROM ${AccessGroupE_1.AccessGroupE.NAME} ag
                JOIN ${CtrlAccessE_1.CtrlAccessE.NAME} ca ON ca.id = ag.ctrl_access_id
    ...
    async addCtrlAccessToGroup(idCtrlAccess, idGroup) {
        ...
        if (ok) {
            let resp = null;
            try {
                resp = await this.db(AccessGroupE_1.AccessGroupE.NAME)
                    .insert({
                    group_id: idGroup,
                    ctrl_access_id: idCtrlAccess,
                });
