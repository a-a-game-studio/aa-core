...
const HashFunc = __importStar(require("../../../System/HashFunc"));
...
class UserSQL extends BaseSQL_1.default {
    ...
    async getUserIDsByToken(sToken) {
        ...
        sql = `
            SELECT
                ...
                ut.token
            FROM ${UserE_1.UserE.NAME} u
            LEFT JOIN ${UserTokenE_1.UserTokenE.NAME} ut ON ut.id_user = u.id
        ...
        if (ok) {
            let sql = `
                SELECT  
                    ...
                    u.phone
                FROM ${UserE_1.UserE.NAME} u
                JOIN ${UserTokenE_1.UserTokenE.NAME} ut ON ut.id_user = u.id
    ...
    async faGetTokenByLoginAndPass(sLogin, sPass) {
        ...
        let sql = `
            ...
            JOIN ${UserTokenE_1.UserTokenE.NAME} ut ON u.id=ut.id_user
            WHERE            
                u.login= :login
            AND
                u.pswd= :pswd 
    ...
    async faDelUser(idUser) {
        try {
            ...
            await this.db(UserGroupE_1.UserGroupE.NAME)
                .where({
                id_user: idUser
            })
                .delete();
